...
const fleetManagement = [
  ...
  },
  {
    ...
    list: [
      ...
      {
        title: "Driver Development Centers",
...
const manpowersolutions = [
  ...
  },
  {
    ...
    link: "manufacturing",
    description:
      "Innovative solutions enhancing sustainability in manufacturing.",
    list: [
      {
    ...
    list: [
      {
        ...
        list: [
          { title: "Engineering Consultants", link: "engineering-consultants" },
        ],
      },
      {
...
const businessConsulting = [
  {
    ...
    link: "digital-transformation-expertise",
  },
              ...
              {!isOpen &&
                navItems.map((item) => (
                  <Link
                    ...
                    style={{
                      color:
                        activeMenu == item.title || pathname == item.href
                          ? "#047857"
                          : isTransparent && !isScrolledLanding && !activeMenu
                            ? "#fff"
                            : "#667085",
      ...
      >
        <div className={styles.maincon}>
          {fleetManagement.map((nav, index) => {
            return (
              <div key={index}>
                <Link onClick={onNav} href={`/fleet-management#${nav.link}`}>
                  <h3>
                    <a>{nav.title}</a>
                  </h3>
                </Link>
      ...
      >
        <div className={styles.maincon}>
          {esgSustainability.map((nav, index) => {
            return (
              <div key={index}>
                <Link onClick={onNav} href={`/esg-sustainability#${nav.link}`}>
                  <h3>{nav.title}</h3>
                </Link>
                <ul className={`${styles.noListStyle}`}>
